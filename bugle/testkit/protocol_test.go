...
func runLoop(ctx context.Context, server bugle.Server, responder bugle.Responder, sessionID, workerID string, andonFn func() *bugle.Andon, budgetFn func() *bugle.BudgetActual) error {
	for {
		if ctx.Err() != nil {
			return ctx.Err()
		}

		pullResp, err := server.Pull(ctx, bugle.PullRequest{
		...
		if err != nil {
			pushReq.Status = bugle.StatusBlocked
			pushReq.Fields = []byte(`{"reason":"` + err.Error() + `"}`)
		} else {
			pushReq.Status = bugle.StatusOk
...
func TestProtocol_WorkerAbortsOnAndonDead(t *testing.T) {
	...
	server.OnPull(func(_ bugle.PullRequest) (bugle.PullResponse, error) {
		return bugle.PullResponse{Andon: bugle.AndonDead}, nil
	})
...
func TestProtocol_WorkerPushesBlockedOnResponderFailure(t *testing.T) {
	...
	server.OnPull(func(_ bugle.PullRequest) (bugle.PullResponse, error) {
		n := pullCount.Add(1)
		if n == 1 {
			return bugle.PullResponse{Available: true, Item: "F0", DispatchID: 1, PromptContent: "test"}, nil
		}
	...
	AssertPushStatus(t, server, bugle.StatusBlocked)
}
...
func TestProtocol_BudgetActualIncludedWhenFuncSet(t *testing.T) {
	...
	budgetFn := func() *bugle.BudgetActual {
		return &bugle.BudgetActual{TokensIn: 500, TokensOut: 300}
	}
...
func TestProtocol_AndonIncludedWhenFuncSet(t *testing.T) {
	...
	andonFn := func() *bugle.Andon {
		return &bugle.Andon{Level: bugle.AndonDegraded, Priority: bugle.PriorityDegraded, Message: "82% tokens"}
	}
	...
	AssertPushCount(t, server, 1)
	AssertAndonLevel(t, server, bugle.AndonDegraded)
}
...
func TestProtocol_WorkerIDSentOnEveryPush(t *testing.T) {
	...
	server.OnPull(func(_ bugle.PullRequest) (bugle.PullResponse, error) {
		n := pullCount.Add(1)
		if n <= 3 {
			return bugle.PullResponse{Available: true, Item: "F0", DispatchID: int64(n), PromptContent: "test"}, nil
		}
		return bugle.PullResponse{Done: true}, nil
	})
...
func TestProtocol_MultipleItemsProcessedSequentially(t *testing.T) {
	...
	server.OnPull(func(_ bugle.PullRequest) (bugle.PullResponse, error) {
		n := pullCount.Add(1)
		if n <= 5 {
			return bugle.PullResponse{Available: true, Item: "item", DispatchID: int64(n), PromptContent: "test"}, nil
		}
		return bugle.PullResponse{Done: true}, nil
	})
