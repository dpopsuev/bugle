...
type mockLauncher struct {
	mu      sync.Mutex
	started map[world.EntityID]AgentConfig
	stopped map[world.EntityID]bool
}

func newMockLauncher() *mockLauncher {
	return &mockLauncher{
		started: make(map[world.EntityID]AgentConfig),
		stopped: make(map[world.EntityID]bool),
	}
}

func (m *mockLauncher) Start(_ context.Context, id world.EntityID, config AgentConfig) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.started[id] = config
	return nil
...
func TestFork_CreatesEntity(t *testing.T) {
	...
	id, err := pool.Fork(ctx, "executor", AgentConfig{
		Role:  "executor",
		Model: "sonnet-4",
	}, 0)
	if err != nil {
...
func TestKillAll(t *testing.T) {
	...
	pool.Fork(ctx, "a", AgentConfig{}, 0)
	pool.Fork(ctx, "b", AgentConfig{}, 0)
	pool.Fork(ctx, "c", AgentConfig{}, 0)
...
func TestGet(t *testing.T) {
	...
	id, _ := pool.Fork(ctx, "executor", AgentConfig{Role: "executor"}, 0)
...
func TestFork_WithBudget(t *testing.T) {
	...
	id, _ := pool.Fork(ctx, "executor", AgentConfig{Budget: 100.0}, 0)
...
func TestConcurrentForkKill(t *testing.T) {
	...
	for range 10 {
		...
		go func() {
			defer wg.Done()
			id, err := pool.Fork(ctx, "worker", AgentConfig{}, 0)
			if err != nil {
				t.Error(err)
				return
			}
